...
static void configGPIO(void) {
    ...
    GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, 0x1F); // PF 0, 1,2,3, 4
...
void ProcesarCadena(void) {
    if (UARTCharsAvail(UART0_BASE)) {
        ...
        // motor1: N4,N2; motor2: N3,N5
        if (strcmp(data, "buscando") == 0) {
            GPIOPinWrite(GPIO_PORTN_BASE, 0x3C, 0x30); // pins N 4 y 5, motor1 avanza, motor2 retrocede. gira sobre el eje.
            UARTprintf("buscando...\n");

        }
        else if (strcmp(data, "figRoja") == 0) {
            GPIOPinWrite(GPIO_PORTN_BASE, 0x3C, 0x00); // apaga motores
            UARTprintf("Objeto Encontrado...\n");
            GPIOPinWrite(GPIO_PORTN_BASE, 0x03, 0x01); // Prende led user 1
            SysCtlDelay(120000000 / 3 * 2); // Aproximadamente 1 segundo
            
        }
        else if (strcmp(data, "figAzul") == 0) {
            GPIOPinWrite(GPIO_PORTN_BASE, 0x3C, 0x00); // apaga motores
            UARTprintf("Objeto Encontrado...\n");
            GPIOPinWrite(GPIO_PORTN_BASE, 0x03, 0x02); // Prende led user 2
            SysCtlDelay(120000000 / 3 * 2); // Aproximadamente 1 segundo
        }
        else if (strcmp(data, "figVerde") == 0) {
            GPIOPinWrite(GPIO_PORTN_BASE, 0x3C, 0x00); // apaga motores
            UARTprintf("Objeto Encontrado...\n");
            GPIOPinWrite(GPIO_PORTN_BASE, 0x03, 0x03); // Prende led user 1 y 2
            SysCtlDelay(120000000 / 3 * 2); // Aproximadamente 1 segundo
        }
